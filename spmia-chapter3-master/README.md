...
- 1. Launch the config server directly, by running ** mvn clean package spring-boot:run ** from the project root
- 2. Launch the licenseservice using the below commands ** mvn clean package spring-boot:run** from the project root
- 3. Try accessing /licensingservice/default on the config server
- 4. Try accessing /actuator/env endpoint on the licesning service
- 5. If a config is changed, config server reflects it immediately. For the clients, they have to make a POST request to the /refresh endpoint to reconcile changes
